action :create_or_modify do
  ...
  bb_config.user.add_or_replace_account(BitlbeeConfig::Accounts::Steam.new(handle: new_resource.handle,
...
action :remove do
  bb_config = BitlbeeConfig::Config.from_directory_for_user(node[:bitlbee][:data_dir], new_resource.user)
  account_to_delete = BitlbeeConfig::Accounts::Steam.new(handle: new_resource.handle)
